...
// Cria um card de filme/série com imagem, botões e informações.
export function createCard(item) {
    const card = document.createElement('div');
    card.className = 'movie-card';

    // Se o item tiver progresso, adiciona uma classe extra.
    if (item.progress) {
    ...
    // O iframe fica escondido até o mouse passar por cima.
    ...
    // Se o item tiver progresso, mostra barra de progresso.
    if (item.progress) {
    ...
    card.addEventListener('mouseenter', () => {
        ...
        // Se estiver muito perto da borda, ajusta a origem da animação.
        if (rect.left < 100) {
        ...
        // Espera um pouquinho e depois começa a tocar o vídeo.
